...
export async function voiceRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: z.infer<typeof processAudioSchema> }>(
    ...
    async (request, reply) => {
      try {
        const { userId, audioBase64, conversationStep = 1 } = processAudioSchema.parse(request.body);

        // Transcribe audio using OpenAI Whisper
        const transcript = await transcribeAudio(audioBase64);

        if (!transcript || transcript.trim().length === 0) {
          return reply.code(400).send({ error: 'Could not transcribe audio. Please try again.' });
        }
        ...
        const nextPrompt = await generateNextPrompt(userId, transcript, extractedData, conversationStep);
